...
class HistoryRegistry:
    ...
    def update_history_registry(self, build_info: Dict[str, Any], deltas: Dict[str, int], logger: Optional[logging.Logger] = None) -> None:
        ...
        if len(registry) > self.max_entries:
            ...
            old_entries = registry[:-self.max_entries]
            if old_entries:
                self._archive_old_entries(old_entries, logger)
    ...
    def _archive_old_entries(self, old_entries: List[Dict[str, Any]], logger: Optional[logging.Logger] = None) -> None:
        """
        Archive old registry entries to compressed files.
        
        Args:
            old_entries: List of old entries to archive
            logger: Optional logger instance for output
        """
        import zipfile
        ...
        try:
            ...
            if logger:
                logger.info(f"📦 Archived {len(old_entries)} old entries to: {archive_path}")
            
        except Exception as e:
            if logger:
                logger.error(f"❌ Failed to archive old entries: {e}")
